...
let
  ...
  # mkGraph: `edges` is wrapped with lib.unique (the registry.nix:74 dedup), and
  ...
  # `base` is the store the lib.fix reads through (already pruned of any dead key);
  ...
  spliceStore =
    ...
    let
      spliced = base // lib.fix (s: lib.genAttrs revCone (id: recompute accessor' (base // s) id));
      keepSet = lib.genAttrs keep (_: true);
    in
    lib.filterAttrs (id: _: keepSet ? ${id}) spliced;
in
{
  inherit mkAccessor;
  ...
  #
  #   - newEdges is DEDUPED (lib.unique) to match the mkGraph edge contract
  ...
  #   - nodes' = lib.unique (nodes ++ newEdges) — grows if a new target appears.
  ...
  #     subgraph is SUB-BUILT (a forward lib.fix over accessor') and spliced into
  ...
  applyEdgeDelta =
    ...
    let
      ...
      # `s.z` throws "missing").
      priorSet = lib.genAttrs accessor.nodes (_: true);
      forwardReach = lib.unique (
        newEdgesU ++ lib.concatMap (graph.reachableFrom { edges = edges'; }) newEdgesU
      );
      ...
      nodes' = lib.unique (accessor.nodes ++ newProducers);
      accessor' = mkAccessor {
        edges = edges';
        nodes = nodes';
        inherit (accessor) nodeData parent;
      ...
      revCone = lib.unique ([ changedId ] ++ graph.dependentsOf checkedAccessor changedId);
      producerStore = lib.fix (
        s: lib.genAttrs newProducers (id: recompute checkedAccessor (ctx.store // s) id)
      );
      ...
      rehash = lib.unique (revCone ++ newProducers);
      trace' =
        ctx.trace
        // lib.genAttrs rehash (id: {
