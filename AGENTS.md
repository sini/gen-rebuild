...
- **Radul & Sussman (2009), *Art of the Propagator*** — provenance (§6.1 support) and the name `kick-out!` for `retract` (§6.2), both **name-faithful only**: no TMS, no merge-lattice, no worldviews (`lib/provenance.nix:10-13`, `lib/structural.nix:13-22`). ★ **`kick-out!` is reversible and deletes nothing** — it marks a premise out of the current worldview (`mark-premise-out!` + alert), with the dual `bring-in!` defined three lines later to restore it, after which the TMS query only falls back to a weaker inference. `retract` genuinely deletes a node, so it is gen-rebuild's own operation wearing Radul's name — not "the destructive half" of his, which has no halves. The delete mechanism is Acar §4.5.
